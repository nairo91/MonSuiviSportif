...
import { BarChart3, Brain, Dumbbell, History, Home, Settings } from "lucide-react";
...
export function BottomNav() {
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-0 z-40 flex justify-center px-4 pb-4 md:pb-6">
      <nav className="pointer-events-auto flex w-full max-w-[430px] items-center justify-between rounded-[30px] border border-white/10 bg-[linear-gradient(180deg,rgba(18,24,40,0.92),rgba(8,12,24,0.9))] px-2 py-3 shadow-[0_30px_70px_-38px_rgba(0,0,0,0.92)] backdrop-blur-2xl safe-bottom">
        {items.map((item) => {
          const active =
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                "flex min-w-[52px] flex-col items-center gap-1 rounded-[22px] px-2 py-2 text-[10px] font-semibold transition",
                active
